...
def evaluate_matches(A_OSE, save_WI, precision_ths, matches):
    ...
    OW_precision = {th : {} for th in precision_ths}
    OW_precision_wo_unkn = {th : {} for th in precision_ths}
        ...
        for oi, overlap_th in enumerate(overlaps):
            ...
            for li, label_name in enumerate(CLASS_LABELS):
                        ...
                        for pred in gt['matched_pred']:
                            ...
                            if overlap > overlap_th:
                                ...
                                if cur_match[gti]:
                                    ...
                                    # append false positive without unknown
                                    if pred['pred_id'] != 3000 and save_WI and gt['label_id'] == 3000:
                                        A_OSE += 1
                ...
                if has_gt and has_pred:
                    ...
                    if save_WI:
                        score_arg_sort_wo_unkn      = np.argsort(y_true_wo_unkn)
                        y_score_wo_unkn_sorted      = y_score_wo_unkn[score_arg_sort_wo_unkn]
                        y_true_wo_unkn_sorted       = y_true_wo_unkn[score_arg_sort_wo_unkn]
                        y_true_wo_unkn_sorted_cumsum = np.cumsum(y_true_wo_unkn_sorted)
                        # unique thresholds
                        (thresholds_wo_unkn,unique_indices_wo_unkn) = np.unique( y_score_wo_unkn_sorted , return_index=True )
                        num_prec_recall_wo_unkn = len(unique_indices_wo_unkn) + 1
                        # prepare precision recall
                        num_examples_wo_unkn      = len(y_score_wo_unkn_sorted)
                        
                        num_true_examples_wo_unkn = y_true_wo_unkn_sorted_cumsum[-1] if len(y_true_wo_unkn_sorted_cumsum) > 0 else 0
                        precision_wo_unkn         = np.zeros(num_prec_recall_wo_unkn)
                        recall_wo_unkn           = np.zeros(num_prec_recall_wo_unkn)
                        
                        for idx_res,idx_scores in enumerate(unique_indices_wo_unkn):
                            cumsum = y_true_wo_unkn_sorted_cumsum[idx_scores-1]
                            tp = num_true_examples_wo_unkn - cumsum
                            fp = num_examples_wo_unkn      - idx_scores - tp
                            fn = cumsum + hard_false_negatives
                            p  = float(tp)/(tp+fp)
                            r  = float(tp)/(tp+fn)
                            precision_wo_unkn[idx_res] = p
                            recall_wo_unkn[idx_res] = r
                    
                    if label_name != "unknown":
                        for recall_level in precision_ths:
                            try:
                                idx1 = np.where(recall-recall_level < 0)[0][0]
                                idx2 = np.where(recall-recall_level > 0)[0][-1]
                                OW_precision[recall_level][label_name] = ((precision[idx2]-precision[idx1])/(recall[idx2]-recall[idx1]))*(recall_level-recall[idx2])+precision[idx2]
                            except:
                                OW_precision[recall_level][label_name] = 0
                                
                            if save_WI:    
                                try:
                                    idx1_wo_unkn = np.where(recall_wo_unkn-recall_level < 0)[0][0]
                                    idx2_wo_unkn = np.where(recall_wo_unkn-recall_level > 0)[0][-1]
                                    OW_precision_wo_unkn[recall_level][label_name] = ((precision_wo_unkn[idx2_wo_unkn]-precision_wo_unkn[idx1_wo_unkn])/(recall_wo_unkn[idx2_wo_unkn]-recall_wo_unkn[idx1_wo_unkn]))*(recall_level-recall_wo_unkn[idx2_wo_unkn])+precision_wo_unkn[idx2_wo_unkn]
                                except:
                                    OW_precision_wo_unkn[recall_level][label_name] = 0
    ...
    return ap, OW_precision, OW_precision_wo_unkn, A_OSE
...
def assign_instances_for_scan(task, pred: dict, gt_file: str):
...
def evaluate(config_general,A_OSE, preds: dict, gt_path: str, output_file: str, dataset: str = "scannet"):
    #train_mode,save_WI,save_precision_in,precision_ths, task, A_OSE,querie_feats,save_KN_UKN_distance, 
    global CLASS_LABELS
...
# 488, 540, 562, 570, 572, 581, 609, 748, 776, 1156, 1163, 1164, 1165, 1166, 1167, 1168, 1169, 1170, 1171, 1172, 1173, 1174, 1175, 1176, 1178, 1179, 1180, 1181, 1182, 1183, 1184, 1185, 1186, 1187, 1188, 1189, 1190, 1191,3000))
    ...
    for i,(k,v) in enumerate(preds.items()):
        ...
        gt2pred, pred2gt = assign_instances_for_scan(config_general.OW_task, v, gt_file)
        matches[matches_key] = {}
        ...
        sys.stdout.flush()
    print('')
    ap_scores, OW_precision, OW_precision_wo_unkn, A_OSE = evaluate_matches(A_OSE, config_general.save_WI_AOSE,config_general.save_precision_at, matches)
    ...
    if config_general.save_WI_AOSE and not config_general.train_mode:
        
        if not os.path.exists(config_general.save_features_in+config_general.OW_task):
            os.makedirs(config_general.save_features_in+config_general.OW_task)
        
        if not os.path.exists(config_general.save_precision_in):
            os.makedirs(config_general.save_precision_in)
        
        file_path = config_general.save_precision_in+"precision_"+config_general.OW_task+".yml"
        with open(file_path, 'w') as fp:
            json.dump(OW_precision, fp,  indent=4)

        file_path = config_general.save_precision_in+"precision_wo_unkn_"+config_general.OW_task+".yml"
        with open(file_path, 'w') as fp:
                json.dump(OW_precision_wo_unkn, fp,  indent=4)
        
        WI = {}
        for th in list(OW_precision.keys()):
            WI[th] = ((np.array(list(OW_precision_wo_unkn[th].values())).mean())/(np.array(list(OW_precision[th].values())).mean()))-1
        
        file_path = config_general.save_precision_in+"eval_critiria/WI_"+config_general.OW_task+".yml"
        with open(file_path, 'w') as fp:
                json.dump(WI, fp,  indent=4)
                
        file_path = config_general.save_precision_in+"eval_critiria/A_OSE_"+config_general.OW_task+".yml"
        with open(file_path, 'w') as fp:
                json.dump(A_OSE, fp,  indent=4)
    
    if config_general.save_KN_UKN_tSNE:
        file_path_p = config_general.save_features_in+"train_set/"+config_general.OW_task if config_general.save_energy_training_dataset else config_general.save_features_in+"val_set/"+config_general.OW_task
        ...
        #saving the orm of unknown known features difference
        mask = label_ids==200
        features_ukn = features[mask]
        features_kn = features[~mask]
        center_kn_feats = np.mean(features_kn, axis = 0)
        center_ukn_feats = np.mean(features_ukn, axis = 0)
        dist_1 = np.linalg.norm(center_kn_feats-center_ukn_feats, 1)
        dist_2 = np.linalg.norm(center_kn_feats-center_ukn_feats, 2) 
        coine = np.dot(center_kn_feats,center_ukn_feats)/(np.linalg.norm(center_kn_feats, 2)*np.linalg.norm(center_ukn_feats, 2))
        dist = [["norm 1", dist_1], ["norm 2", dist_2], ["coine", coine]]
        if not os.path.exists(file_path_p+"/output"):
            os.makedirs(file_path_p+"/output")
        file_path = file_path_p+"/output"+"/norm_between_centers"+".yml"
        with open(file_path, 'w') as fp:
                json.dump(dist, fp,  indent=4)
        #plotting teh tSNE for features         
        features_final = TSNE(perplexity=30).fit_transform(features) 
        palette = np.array(sb.color_palette("hls", 201))
        palette[200] = np.array([0,0,0]) #unknown is assigned a black color
        f = plt.figure(figsize=(8, 8))
        ax = plt.subplot(aspect='equal')
        sc = ax.scatter(features_final[:,0], features_final[:,1], lw=0, s=20, c=palette[label_ids.astype(np.int)])
        plt.savefig(file_path_p+"/output"+'/tSNE_plot.png')
        f1 = plt.figure(figsize=(8, 8))
        ax1 = plt.subplot(aspect='equal')
        sc1 = ax1.scatter(features_final[:,0][mask], features_final[:,1][mask], lw=0, s=20, c=palette[label_ids[mask].astype(np.int)])
        plt.savefig(file_path_p+"/output"+'/tSNE_plot_unkn.png')
        f2 = plt.figure(figsize=(8, 8))
        ax2 = plt.subplot(aspect='equal')
        sc2 = ax2.scatter(features_final[:,0][~mask], features_final[:,1][~mask], lw=0, s=20, c=palette[label_ids[~mask].astype(np.int)])
        plt.savefig(file_path_p+"/output"+'/tSNE_plot_kn.png')
        ...
        #plot the tSNE for means
        per_cls_mean_feats_final = TSNE(perplexity=30).fit_transform(mean_feats_labels[:,:-1]) 
        f3 = plt.figure(figsize=(8, 8))
        ax3 = plt.subplot(aspect='equal')
        sc3 = ax3.scatter(per_cls_mean_feats_final[:,0], per_cls_mean_feats_final[:,1], lw=0, s=20, c=palette[(mean_feats_labels[:,-1].flatten().numpy()).astype(np.int)])
        plt.savefig(file_path_p+"/output"+'/tSNE_means_plot.png')  
        # mean_feats_dist = mean_feats_labels[mean_feats_labels[:,-1]!=200]-mean_feats_labels[mean_feats_labels[:,-1]==200]
        # mean_feats_norm_1 = torch.linalg.norm(mean_feats_dist, ord=1, dim=-1).tolist()
        # mean_feats_norm_2 = torch.linalg.norm(mean_feats_dist, ord=2, dim=-1).tolist()
        # dist = {"norm1": mean_feats_norm_1, "norm2": mean_feats_norm_2}
        # file_path = file_path_p+"/output"+"/per_class_norm_between_centers"+".yml"
        # with open(file_path, 'w') as fp:
        #         json.dump(dist, fp,  indent=4)         
        end = time.time()
        ...
        file_path = file_path_p+"/output"+"/plotting_time"+".yml" 
        with open(file_path, 'w') as fp:
